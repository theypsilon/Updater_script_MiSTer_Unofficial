...
if false ; then # @DISABLED BEGIN: These GitHub API calls are disabled for the Unofficial updater, since we have cores from different user spaces, and we don't want to waste the limited number of free GitHub api calls per hour. No additional identation inserted in order to avoid conflicts with upstream.
...
function checkCoreURL {
	...
	case "$CORE_URL" in
		*SD-Installer*)
			...
			;;
		*)
			# For the Unofficial Updater we need to keep relying on HTML parsing, since we haven't done GitHub API calls. Following line is addressing this:
			BRANCH_NAME=$(curl $CURL_RETRY $SSL_SECURITY_OPTION -sSLf "${CORE_URL}/branches" | grep "branch-name" | head -n1 | sed 's/.*>\(.*\)<.*/\1/')
	...
	RELEASE_URLS=$(echo ${RELEASES_HTML} | grep -oE '/'${DOMAIN_URL}'/[a-zA-Z0-9./_-]*_[0-9]{8}[a-zA-Z]?(\.rbf|\.7z|\.zip)?')
